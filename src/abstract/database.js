  ...
  const DBM = {
    db: {
      ...
      async query (modelName, parent, query) {
        const res = await abs.query(await resolveModel(modelName), parent, query)

        return Promise.all(res.map(async res => Pandemonica(res, abs, await resolveModel(modelName))))
      },
      async native (modelName) {
        return abs.native(await resolveModel(modelName))
      },
      async fromNative (modelName, results) {
        const res = abs.fromNative(await resolveModel(modelName), results)
        return Promise.all(res.map(async res => Pandemonica(res, abs, await resolveModel(modelName))))
      }
    },
